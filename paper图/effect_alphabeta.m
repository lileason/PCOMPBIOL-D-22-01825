...
r1 = (N.*(w-1)+rd.*((w.*x0-x0+1).^(N-1)-1))./(w.*(w.*x0-x0+1).^(N-1)-1);
e1 = (r1.*w-rd).*(N-1).*(w.*theta+1).^(N-2).*(w-1)./(r1-alpha1)./(beta1-r1)./(w.*(w.*theta+1).^(N-1)-(1+theta).^(N-1));
r2 = (N.*(w-1)+rd.*((w.*x0-x0+1).^(N-1)-1))./(w.*(w.*x0-x0+1).^(N-1)-1);
e2 = (r2.*w-rd).*(N-1).*(w.*theta+1).^(N-2).*(w-1)./(r2-alpha2)./(beta2-r2)./(w.*(w.*theta+1).^(N-1)-(1+theta).^(N-1));
...
for i = 1:floor(1/h)-1
    for j = 1:floor(1/h)-1
        x_0 = h*i;
        r_0 = 1+4*h*j;
        [t,x]=ode23(@equfd2,[0,tf],[x_0;r_0],options,epsilon,theta,rd,N,c,a,alpha2,beta2);
        X=x(:,1);
        R=x(:,2);
        if X(end) > 1-exp(-6)
            arrowPlot(X,R,'number', 5,'color', [1 0.41176 0.70588], 'LineWidth', 0.2, 'scale', 0.1);
...
%         x_0 = h*i;
%         r_0 = 1.5+2*h*j;
%         [t,x]=ode45(@equsin,[0,tf],[x_0;r_0],options,a,delta,epsilon,theta,rd,N,c);
%         X=x(:,1);
%         R=x(:,2);
%         if X(end) > 1-exp(-6)
%             arrowPlot(X,R,'number', 5,'color', [1 0.41176 0.70588], 'LineWidth', 0.2, 'scale', 0.1);
...
for i = 1:floor(1/h)-1
    for j = 1:floor(1/h)-1
        x_0 = h*i;
        r_0 = 1+4*h*j;
        [t,x]=ode23(@equsin2,[0,tf],[x_0;r_0],options,a,delta,epsilon,theta,rd,N,c,alpha2,beta2);
        X=x(:,1);
        R=x(:,2);
        if X(end) > 1-exp(-6)
            arrowPlot(X,R,'number', 5,'color', [1 0.41176 0.70588], 'LineWidth', 0.2, 'scale', 0.1);
