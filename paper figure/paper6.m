...
%         x_0 = h*i;
%         r_0 = 1.5+2*h*j;
%         [~,x] = ode45(@equfd,[0,tend],[x_0;r_0],options,epsilon,theta,rd,N,c,a);
%         X=x(:,1);
%         R=x(:,2);
%         if X(end) > 1-exp(-6)
...
% figure(2)
% f1 = plot(xx,rr1,'b^','LineWidth', 1);
% hold on
% f2 = plot(xx,rr2,'r^','LineWidth', 1);%两个平衡点
% hold on
...
% xlabel('frequency of cooperators,x');
% ylabel('multiplication of cooperators,r_{c}');
% legend([f1 f2],'fixed point 1','fixed point 2');
% box on
% hold off
% 
figure(3)
...
%         x_0 = h*i;
%         r_0 = 1.5+2*h*j;
%         [~,x]=ode45(@equst,[0,tend],[x_0;r_0],options,a,delta,epsilon,theta,rd,N,c);
%         X=x(:,1);
%         R=x(:,2);
%         if X(end) > 1-exp(-6)
%             arrowPlot(X,R,'number', 5,'color', [1 0.41176 0.70588], 'LineWidth', 0.2, 'scale', 0.1);
...
%         x_0 = h*i;
%         r_0 = 1.5+2*h*j;
%         [~,x] = ode45(@equfd1,[0,tend],[x_0;r_0],options,epsilon,theta,rd,N,c,a);
%         X=x(:,1);
%         R=x(:,2);
%         if X(end) > 1-exp(-6)
%             arrowPlot(X,R,'number', 5,'color', [1 0.41176 0.70588], 'LineWidth', 0.2, 'scale', 0.1);
...
% hold on
% f5 = plot(xx,rr5,'b^','LineWidth', 1);
% hold on
% f6 = plot(xx,rr7,'g^','LineWidth', 1);
% hold on
% f7 = plot(xx,rr6,'r^','LineWidth', 1);%3个平衡点
% hold on
...
% xlabel('frequency of cooperators,x');
% ylabel('multiplication of cooperators,r_{c}');
% legend([f5 f6 f7],'fixed point 1','fixed point 2','fixed point 3');
% box on
% hold off
